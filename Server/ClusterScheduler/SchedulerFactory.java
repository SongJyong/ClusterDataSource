...
public class SchedulerFactory {
    ...
    public int getScheduleAddress(int requestId){
        int componentAddress = -2;
        if (ClusterConnectionPool.availableLinkedList.length == 0){
            System.out.println("Connection Error : anyone component pool doesn't exist");
            return componentAddress;
        }
        componentAddress = affinityScheduler.getScheduleAddress(requestId); // affinity 최우선 확인
        if (componentAddress == -1){
            componentAddress = roundRobinScheduler.getScheduleAddress(); // 다른 조건이 없으면 기본 round-robin 스케쥴러 이용
        }
        return componentAddress;
    }
